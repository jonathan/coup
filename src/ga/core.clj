...
(defn gen-tournaments
  ...
  [population tourney]
    (go (>! tourney (partition tourney-size population)))
    (go (>! tourney (repeatedly (/ population-size winner-size) (fn [] (repeatedly tourney-size (fn [] (rand-nth population)))))))
    (go (>! tourney (repeatedly (/ population-size winner-size) (fn [] (repeatedly tourney-size (fn [] (rand-nth population))))))))

;(defn top-winners
;  "Returns the a number of winners from the given tournament seq."
;  [tournament winners]
;  (let [tourney-results (map #(assoc % :fitness (fitness (:chromosome %))) (go (<! tournament)))]
;    (go (>! winners (take winner-size (sort-by :fitness > tourney-results))))))
;
;(defn crossover
...
;            winners (chan)
;            new-pop (chan)]
;            (gen-tournaments population tourney)
;            (map top-winners tourney winners)
;            (go (<! new-pop (flatten (map #(crossover (first %) (second %)) (go (<! winners))))))
